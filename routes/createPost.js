...
router.get("/",(request,response) => {
    ...
    if (request.socket.remoteAddress in cookie){
        ...
        connection.query(
        "INSERT INTO `posts`(`id`, `mail`, `title`, `description`, `report`, `category`, `visibility`, `anonymous`, `comment`, `draft`, `datetime`, `name`, `picture`) VALUES ('"+hash+"','"+cookie[request.socket.remoteAddress]["mail"]+"','','','','0','0','0','0','1','"+(new Date()).toLocaleDateString()+" "+(new Date()).toLocaleTimeString()+"','"+cookie[request.socket.remoteAddress]["name"]+"','"+cookie[request.socket.remoteAddress]["picture"]+"')",(err, result, fields)=>{
            response.redirect("edit/"+hash);
        });
        connection.query("INSERT INTO `notifications`(`mail`, `title`, `message`, `button`, `icon`, `color`, `datetime`, `isread`) VALUES ('"+cookie[request.socket.remoteAddress]["mail"]+"','Post Creation','Created A New Post, Saved In Draft.','"+'<a class="btn btn-sm btn-secondary" href="../edit/'+hash+'">Edit Post</a>'+"','fa-solid fa-file-pen','primary','"+(new Date()).toLocaleDateString()+" "+(new Date()).toLocaleTimeString()+"','0')",(err, result, fields)=>{});
        return;
    }
